...
import { crearPublicacionAction } from "../actions/crear-publicacion";
...
export function useCrearPublicacionForm() {
  ...
  const validateField = (
    name: keyof typeof crearPublicacionSchema.shape,
    value: string,
  ) => {
    const fieldSchema = crearPublicacionSchema.shape[name];
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = crearPublicacionSchema.safeParse(data);
