...
import { editarRecursoEducativoExistente } from "../services/editar-recurso-educativo-existente";
import type { RecursoEducativoActionState } from "./recurso-educativo-action-state";

export async function editarRecursoEducativoAction(
  ...
  const parseResult = editarRecursoEducativoSchema.safeParse({
    ...rawData,
  ...
  try {
    const recurso = await editarRecursoEducativoExistente(
      usuario.id,
      usuario.rol,
      parseResult.data,
    );
    ...
    revalidatePath(`/aprende/${recurso.slug}/editar`);
    redirectPath = `/aprende/${recurso.slug}`;
  } catch (error) {
    const errorMsg =
      error instanceof Error
        ? error.message
        : "Error al editar el recurso educativo.";
    return {
      success: false,
      error: errorMsg,
      fields: rawData,
